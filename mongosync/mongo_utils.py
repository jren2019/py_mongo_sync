...
def get_version(arg):
    ...
    if isinstance(arg, pymongo.MongoClient):
        ...
        if ("mongodb+srv" in arg):
            with pymongo.MongoClient(arg) as mc:
...
def get_replica_set_name(host, port, **kwargs):
    ...
    try:
        if port == None:
            ...
            mc = pymongo.MongoClient(host, connect=True, serverSelectionTimeoutMS=3000)
            if username and password and authdb:
                mc[authdb].authenticate(username, password)
