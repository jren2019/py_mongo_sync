...
class CommonSyncer(object):
    ...
    def _split_coll(self, namespace_tuple, n_partitions):
        ...
        max_chunk_size = int(
            ((collstats['count'] / (n_partitions - 1) - 1) * 2 * collstats['avgObjSize']) / 1024 / 1024)

        if max_chunk_size <= 0:
            return []

        res = db.command('splitVector', ns, keyPattern={'_id': 1}, maxSplitPoints=n_points, maxChunkSize=max_chunk_size,
